...
public class Admin_User {
    ...
    private ResponseEntity<?> LoginAdminUser(@Valid @RequestBody SELLER seller){



        Optional<USER> ad = repository.findByEmail(seller.getEmail());
        if (!ad.isPresent())
        {
        ...
        //validating password
        if(!ad.get().getPassword().equals(seller.getPassword())){
            return new ResponseEntity<>("Invalid Crediantials", HttpStatus.BAD_REQUEST);
        }


        UserDetails appUser = myUserDetailsService.loadUserByUsername(ad.get().getEmail());
